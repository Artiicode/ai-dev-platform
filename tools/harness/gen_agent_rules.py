...
| "작업 세션/tmux 띄워줘" | `./harness start [세션이름]` (tmux window=`dev`; 좌 claude/우상 tmux 치트시트+자유셸/우하 usage; 하네스·claude 권한을 화살표 메뉴로 고르고 `.harness-local.json` 에 저장. claude 는 clone한 플랫폼 디렉토리에서 실행, `--cwd` 로 변경) |
