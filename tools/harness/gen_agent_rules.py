...
사용:
  python tools/harness/gen_agent_rules.py            # 플랫폼 루트
  python tools/harness/gen_agent_rules.py --node my_proj   # 노드 레벨
...
HEADER = (
    "<!-- AUTO-GENERATED by tools/harness/gen_agent_rules.py (v%s) — DO NOT EDIT BY HAND.\n"
    "     진실원본: platform/prompts/global-system.md (+ platform/policies/).\n"
    "     재생성: python tools/harness/gen_agent_rules.py%s -->\n"
)
...
| "오늘 할 일 추가 / 스탠드업" | 할일 `./harness standup --add-task "<할 일>"`(또는 `/add-task`) · 진행 `--add "<항목>"` · 요약 `--today/--tomorrow` · 보기 `--show`. **노드 생략=플랫폼 개인 일일 플랜**(`harness start` 의 subtask 창), `<node>` 지정=프로젝트 standup. 전날 미완료(`- [ ]`)·내일계획은 오늘로 자동 carry-over, 비면 "없음". **개인 일일 플랜은 오늘 각 프로젝트의 worklog/standup 을 자동 집계해 `[프로젝트 진행]`에 표시** — 그러니 프로젝트 작업은 `append_worklog`(또는 `harness standup <node> --add`)로만 남기면 일일 플랜에 자동 반영된다(플랫폼 플랜에 따로 쓸 필요 없음) |
